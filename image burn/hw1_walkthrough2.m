...
green_image = zeros(size(img)); green_image(:, :, 2) = red_channel; figure, imshow(green_image);
%green_image = ???;

blue_channel = img(:, :, 3); figure, imshow(blue_channel);
blue_image = zeros(size(img)); blue_image(:, :, 3) = red_channel; figure, imshow(blue_image);
%blue_image = ???;
