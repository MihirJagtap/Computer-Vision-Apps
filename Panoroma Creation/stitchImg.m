...
%               get you started.
    ...
    for n = 1:num_imgs
        ...
        %
        [~ , H_3x3] = runRANSAC(kp_n, kp_stitched, 100, 3);

        [mask, img_bwi] = backwardWarpImg(img_n, inv(H_3x3), [size(stitched_img, 2), size(stitched_img, 1)]);

        stitched_img = blendImagePair(stitched_img, stitch_mask, img_bwi, mask, "blend");


        % I am adjusting the stitch mask here
        stitch_mask = (stitch_mask + mask) > 0; % the stitch mask should be 
        %added with mask and checked for > 0
